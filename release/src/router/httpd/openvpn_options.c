...
add_option (char *p[], int line, int unit)
{
	...
	{
		sprintf(buf, "vpn_client_if", unit);
	...
	{
		sprintf(buf, "vpn_client_proto", unit);
		nvram_set(buf, p[1]);
	}
	else if  (streq (p[0], "remote") && p[1])
	{
		sprintf(buf, "vpn_client_addr", unit);
		nvram_set(buf, p[1]);

		sprintf(buf, "vpn_client_port", unit);
		if(p[2])
			nvram_set(buf, p[2]);
		else
			nvram_set(buf, "1194");
	...
	{
		sprintf(buf, "vpn_client_retry", unit);
	...
	{
		sprintf(buf, "vpn_client_comp", unit);
		if(p[1])
			nvram_set(buf, p[1]);
		else
	...
	{
		sprintf(buf, "vpn_client_crypt", unit);
		nvram_set(buf, "tls");
		if (streq (p[1], INLINE_FILE_TAG) && p[2])
		{
			sprintf(buf, "vpn_crt_client_ca", unit);
	...
	else if  (streq (p[0], "cert") && p[1])
	{
		if (streq (p[1], INLINE_FILE_TAG) && p[2])
		{
			sprintf(buf, "vpn_crt_client_crt", unit);
			write_encoded_crt(buf, strstr(p[2], "-----BEGIN"));
		}
		else
		{
	...
	else if  (streq (p[0], "key") && p[1])
	{
		if (streq (p[1], INLINE_FILE_TAG) && p[2])
		{
			sprintf(buf, "vpn_crt_client_key", unit);
			write_encoded_crt(buf, strstr(p[2], "-----BEGIN"));
		}
		else
		{
	...
	else if (streq (p[0], "tls-auth") && p[1])
	{
		if (streq (p[1], INLINE_FILE_TAG) && p[2])
		{
			sprintf(buf, "vpn_crt_client_static", unit);
			write_encoded_crt(buf, strstr(p[2], "-----BEGIN"));
		}
		else
		{
			if(p[2]) {
				sprintf(buf, "vpn_client_hmac", unit);
				nvram_set(buf, p[2]);
			}
	...
	{
		sprintf(buf, "vpn_client_userauth", unit);
		nvram_set(buf, "1");
	}
	else if (streq (p[0], "tls-remote") && p[1])
	{
		sprintf(buf, "vpn_client_tlsremote", unit);
		nvram_set(buf, "1");
		sprintf(buf, "vpn_client_cn", unit);
		nvram_set(buf, p[1]);
	}
	...
	{
		sprintf(buf, "vpn_client_custom", unit);
		add_custom(buf, p);
	}
	return 0;
}
...
void reset_client_setting(int unit){
	...
	sprintf(nv, "vpn_client_custom", unit);
	nvram_set(nv, "");
	sprintf(nv, "vpn_client_comp", unit);
	nvram_set(nv, "no");
	sprintf(nv, "vpn_client_reneg", unit);
	nvram_set(nv, "-1");
	sprintf(nv, "vpn_client_hmac", unit);
	nvram_set(nv, "-1");
	sprintf(nv, "vpn_client_retry", unit);
	nvram_set(nv, "-1");
	sprintf(nv, "vpn_client_cipher", unit);
	nvram_set(nv, "default");
	sprintf(nv, "vpn_client_rgw", unit);
	nvram_set(nv, "0");
	sprintf(nv, "vpn_client_tlsremote", unit);
	nvram_set(nv, "0");
	sprintf(nv, "vpn_client_cn", unit);
	nvram_set(nv, "");
	sprintf(nv, "vpn_client_userauth", unit);
	nvram_set(nv, "0");
	sprintf(nv, "vpn_client_username", unit);
	nvram_set(nv, "");
	sprintf(nv, "vpn_client_password", unit);
	nvram_set(nv, "");
	sprintf(nv, "vpn_client_comp", unit);
	nvram_set(nv, "-1");
...
void parse_openvpn_status(int unit){
	...
	sprintf(buf, "/etc/openvpn/server/status", unit);
	fpi = fopen(buf, "r");

	sprintf(buf, "/etc/openvpn/server/client_status", unit);
	fpo = fopen(buf, "w");

	snprintf(prefix_vpn, sizeof(prefix_vpn), "vpn_server_", unit);
	...
	if(fpi && fpo) {
		while(!feof(fpi)){
			...
			if(!strncmp(buf, "CLIENT_LIST", 11)) {
				...
				snprintf(nv_name, sizeof(nv_name) -1, "vpn_server_if", unit);
