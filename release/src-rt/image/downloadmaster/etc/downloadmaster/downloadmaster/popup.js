...
function LoadingTime(seconds, flag){
	showtext($("proceeding_main_txt"), multiLanguage_all_array[multi_INT][5]+"...");
	...
	if(typeof(seconds) == "number" && seconds >= 0){
		if(seconds != 0){
			showtext($("proceeding_main_txt"), multiLanguage_all_array[multi_INT][6]);
			showtext($("proceeding_txt"), Math.round(y)+"% "+multiLanguage_all_array[multi_INT][5]);
			--seconds;
			setTimeout("LoadingTime("+seconds+", '"+flag+"');", 1000);
		}
		else{
			showtext($("proceeding_main_txt"), translate(multiLanguage_all_array[multi_INT][7]));
