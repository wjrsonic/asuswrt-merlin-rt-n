...
function add_task(i){
	...
	task_code +='<table id="'+j+'" class="taskLongWidth" style="border-bottom:#000 solid 1px; table-layout:fixed;" onclick="selectedTask(this.id);">\n';
		...
		if(status=="Hashing"){
			task_code +='<td style="width:20px;"><img id="status_icon'+j+'" src="images/statusIcon/'+'Initialing.gif" alt="" /></td></tr>\n';
		}
		else{
		...
		//if(type=="FTP"){
			if(ftp_name_index>0){
				...
				var ftp_name="asus-none-ftpname"+j;
			}
		//}
...
function initial_Refresh_time(data){
	...
	if(location.host.split(":")[0] != initial_array[10]){
		document.getElementById("handToPC").href = "http://"+ location.host.split(":")[0] +":" + initial_array[13]+"/downloadmaster/task.asp";
	}
	else{
		document.getElementById("handToPC").href = "http://"+ location.host +"/downloadmaster/task.asp";
	}
