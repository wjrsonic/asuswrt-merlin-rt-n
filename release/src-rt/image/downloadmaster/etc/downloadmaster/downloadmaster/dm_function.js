...
function bt_initialing(status){
	...
	switch(status){
		...
		case "cancel":
		case "paused":
		case "start":if((dm[5] == BT) && (dm[4] == Initialing) ||  (dm[4] == Hashing)){
							Ext.MessageBox.alert(multiLanguage_array[multi_INT][3],multiLanguage_array[multi_INT][0]+"\n"+multiLanguage_array[multi_INT][1]);
							return 1;
						}
						return 0;
...
function Ftp_dm_add_status(){
	...
	if(ftp.test(furl_s)){
		...
		else
		{
			...
			//foot = furl.substr(del_user+1,len-del_user-1);
			furl = "ftp://" + user_pass + foot;
		}
...
function add_task(i){
	...
	else
	{
		...
		if(filename.substring(0,6)=="FTP://" || filename.substring(0,6)=="ftp://"){
			if(ftp_name_index>=0){
				//var ftp_name=filename.substring(6).substring(0,ftp_name_index);
...
function create_downloadInfo(){
	...
	taskLog_code += "<span style='margin-left:17px;' id='taskLog_Size'>-</span></td></tr>\n";
	taskLog_code += "<tr><td>\n";
	taskLog_code += "<span style='margin-left:5px;'>"+multiLanguage_array[multi_INT][53]+"</span>\n";
	taskLog_code += "<span style='margin-left:17px;' id='taskLog_Status'>-</span></td></tr>\n";
	...
	taskLog_code += "<span style='margin-left:17px;' id='taskLog_Downloaded'>-</span></td></tr>\n";
	if (dm[5] != "ED2K"){
	taskLog_code += "<tr><td>\n";
	...
	for(var i = 0; i < dm_array.length ; i++){
		...
		if(filename.substring(0,6)=="FTP://" || filename.substring(0,6)=="ftp://"){
			if(ftp_name_index>=0){
				...
				//var ftp_name=filename.substring(6).substring(0,ftp_name_index); //user:name
			}
			else{
				ftp_name="asus-none-ftpname"+j;
			}
...
function initial_Refresh_time(data){
	...
	if(location.host.split(":")[0] != initial_array[10]){
		if(initial_array[12] == 1 && (location.host.split(":")[0]!=LOCAL_DOMAIN)){
				...
				document.getElementById("helpAddress").href = "http://"+ location.host.split(":")[0] + ":" + initial_array[13]+"/downloadmaster/help.asp";
				}
