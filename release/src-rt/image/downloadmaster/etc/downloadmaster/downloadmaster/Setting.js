...
function update_amule_status(data){
	var amule_status = new Array();
	eval("amule_status="+data);
	//document.getElementById("aMule_server_status_string").style.color= "#ff0000 "; 
	if(amule_status[0]=="disconnected"){
		amule_status_tmp="disconnected";
		$j("#aMule_Disconnection").hide();
		$j("#aMule_Connection").show();
		$j("#aMule_server_status_string").html(multiLanguage_setting_array[multi_INT][71]);
		$j("#aMule_server_status_pic").hide();
	}
	if(amule_status[0]=="disconnected2"){
		amule_status_tmp="disconnected2";
		$j("#aMule_Disconnection").hide();
		$j("#aMule_Connection").show();
		$j("#aMule_server_status_string").html(multiLanguage_setting_array[multi_INT][113]);
		$j("#aMule_server_status_pic").hide();
	}
	else if(amule_status[0]=="connected"){
		amule_status_tmp="connected";
		$j("#aMule_Connection").hide();
		$j("#aMule_Disconnection").show();
		$j("#aMule_server_status_string").html(multiLanguage_setting_array[multi_INT][72]);
		$j("#aMule_server_status_pic").hide();
	}
	else if(amule_status[0]=="connecting"){
		amule_status_tmp="connecting";
		$j("#aMule_Disconnection").show();
		$j("#aMule_Connection").hide();
		$j("#aMule_server_status_string").html(multiLanguage_setting_array[multi_INT][73]);
		$j("#aMule_server_status_pic").show();
	}
	else if(amule_status[0]=="initializing"){
		amule_status_tmp="initializing";
		$j("#aMule_Disconnection").show();
		$j("#aMule_Connection").show();
		$j("#aMule_server_status_string").html(multiLanguage_setting_array[multi_INT][74]);
		$j("#aMule_server_status_pic").hide();
	}
}
...
function get_amule_server_status(){
	...
	url += "?action_mode=" + action_mode + "&download_type=" +type+ "&t=" +Math.random();
	$j.get(url,function(data){update_amule_status(data);});
	}
...
function BTapplyRule(){
	...
	else if(!digit.test(downSpeed) || downSpeed=="")
	Ext.MessageBox.alert(multiLanguage_setting_array[multi_INT][39],multiLanguage_setting_array[multi_INT][47]);
	else if(!digit.test(upSpeed) || upSpeed=="")
...
function callBackGeneral(){
	...
	url += "?action_mode=" + action_mode + "&download_type=" +type+  "&Base_path=" +Base_path+  "&Download_dir=" +Download_dir+  "&DM_dir_renew="+Download_dir_renew+"&Refresh_rate=" +Refresh_rate+  "&Misc_http_x=" +Misc_http_x+  "&Lan_ip=" +Lan_ip+ "&DM_port="+ DM_port +"&DM_port_renew="+DM_port_renew+"&Start_hour="+"00"+"&End_hour="+"23"+ "&Start_minute="+"00"+"&End_minute="+"59"+"&Enable_time="+enableTime+"&DM_language="+lang_tmp+"&Miscr_http_x="+MISCR_HTTP_X+"&Miscr_httpport_x="+MISCR_HTTPPORT_X+"&Day="+Day+"&Enable_time_renew="+Enable_time_renew+ "&Productid="+Productid+"&APPS_DEV="+APPS_DEV+"&WAN_IP="+WAN_IP+"&DDNS_ENABLE_X="+DDNS_ENABLE_X+"&DDNS_HOSTNAME_X="+DDNS_HOSTNAME_X+"&MAX_ON_HEAVY="+MAX_ON_HEAVY+"&MAX_QUEUES="+MAX_QUEUES+"&MAX_ON_ED2K="+MAX_ON_ED2K+"&RFW_ENABLE_X="+RFW_ENABLE_X+"&DEVICE_TYPE="+DEVICE_TYPE+"&dm_radio_date_x="+dm_radio_date_x+"&dm_radio_time_x="+dm_radio_time_x+"&dm_radio_time2_x="+dm_radio_time2_x+"&misc_seeding_x="+misc_seeding_x+"&misc_seeding_x_renew="+misc_seeding_x_renew+"&t=" +Math.random();
	if(MISCR_HTTPPORT_X == DM_port || MISCR_HTTPSPORT_X == DM_port){
		Ext.MessageBox.alert(multiLanguage_setting_array[multi_INT][39],multiLanguage_setting_array[multi_INT][52]);
		}
	else if(DM_port>65535 || DM_port<1024 || DM_port == 9092 || DM_port == 6789 || DM_port == 3490 || DM_port == 80 || DM_port == 8082 || DM_port == "" || DM_port == 8200 || DM_port == 3689 || DM_port == 3678 || DM_port == 3567 || DM_port == 3568 || DM_port == 3569){
